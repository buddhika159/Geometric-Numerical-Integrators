classdef semiexplicit_Broyden
    ...
    methods (Static)
        ...
        function [q, p, NW_itrs, Max_Error, Max_Defect, method_time] = base2(gradq, gradp, q_int, p_int, dt, itr, tol, Max_NW_Iter)
            ...
            for i=1:itr
                mu_0 = zeros(2*dim,1);
                J_inv_0 = 1/4*eye(2*dim);
                
                z_in = z(:,i);   % zeta + A^T*mu_0
                ...
                proj = [qB;pB;xB;yB]; % Phi(zeta + A^T*mu_0) + A^T*mu_0
            
                A_val_0 = A(proj);  % A(proj) = f(mu_0)            
                    
                mu_1 = mu_0 - J_inv_0 * A_val_0;
                error = norm(mu_1 - mu_0);
                ...
                while (error > tol && NW_i < Max_NW_Iter)
                    ...
                    proj = [qB;pB;xB;yB] + [mu_1;-mu_1];
                    
                    % Newton itterations
                    
                    A_val_1 = A(proj);  
                    
                    J_inv_1 = J_inv_0 + ((mu_1 - mu_0) - J_inv_0 * (A_val_1 - A_val_0)) /...
                                        ((mu_1 - mu_0)' * J_inv_0 * (A_val_1 - A_val_0)) *...
                                         (mu_1 - mu_0)' * J_inv_0;
                                    
                    mu_2 = mu_1 - J_inv_1 * A_val_1;
                    
                    J_inv_0 = J_inv_1;
                    A_val_0 = A_val_1;
                    
                    error = norm(mu_2 - mu_1) ;
            ...
            disp('2nd order semiexplicit Broyden done')
        ...
        function [q, p, NW_itrs, Max_Error, Max_Defect, method_time] = TJ4(gradq, gradp, q_int, p_int, dt, itr, tol, Max_NW_Iter)
            ...
            disp('4th order semiexplicit Broyden Triple Jump done')
        ...
        function [q, p, NW_itrs, Max_Error, Max_Defect, method_time] = TJ6(gradq, gradp, q_int, p_int, dt, itr, tol, Max_NW_Iter)
            ...
            disp('6th order semiexplicit Broyden Triple Jump done')
        ...
        function [q, p, NW_itrs, Max_Error, Max_Defect, method_time] = Yo6(gradq, gradp, q_int, p_int, dt, itr, tol, Max_NW_Iter)
            ...
            disp('6th order semiexplicit Broyden Yoshida done')
        ...
        function [q, p, NW_itrs, Max_Error, Max_Defect, method_time] = Su4(gradq, gradp, q_int, p_int, dt, itr, tol, Max_NW_Iter)
            ...
            disp('4th order semiexplicit Broyden Suzuki done')
        ...
        function [q, p, NW_itrs, Max_Error, Max_Defect, method_time] = Su6(gradq, gradp, q_int, p_int, dt, itr, tol, Max_NW_Iter)
            ...
            for i=1:itr
                ...
                proj = [qB;pB;xB;yB] + [mu_0;-mu_0]; % Phi(zeta + A^T*mu_0) + A^T*mu_0
            ...
            disp('6th order semiexplicit Broyden Suzuki done')
